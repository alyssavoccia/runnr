...
const ImportPage = () => {
  ...
  const { importWorkouts, existingIds } = useWorkouts();
  const [source, setSource] = useState("garmin");
  const [workouts, setWorkouts] = useState(null);

  const handleImport = async (incoming) => {
    await importWorkouts(incoming);
    navigate("/app");
  };
  ...
  return (
    <>
      ...
      {workouts && (
        <ImportWorkoutSelector
          workouts={workouts}
          existingIds={existingIds}
          onBack={() => setWorkouts(null)}
